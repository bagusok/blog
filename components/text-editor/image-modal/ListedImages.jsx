...
export default function ListedImages({ onAddImage }) {
  ...
  } = useSwr('/api/v1/user/post/get-images?type=s3', fetcher);

  const {
    data: imageCloudinary,
    error: errorCloudinary,
    isLoading: isLoadingCloudinary,
  } = useSwr('/api/v1/user/post/get-images?type=cloudinary', fetcher);
  ...
  const handleRadio = (e) => {
    setValue({ url: e.url, path: e.path, type: e.type });
  };
  return (
    <div className="w-screen px-3 min-h-screen max-h-full bg-slate-300/50 top-0 flex justify-center items-center z-30 fixed right-0">
      ...
      <div className="md:w-2/3 md:h-96 bg-white rounded-md p-3 h-96 relative overflow-hidden z-40">
        <div className="flex items-center justify-between mb-3">
          ...
          <button type="button" className="text-lg font-bold mr-3" onClick={() => setShowModalUpload(false)}>
            X
          </button>
        </div>
        ...
        <Tab.Group>
          <Tab.List className="flex gap-3 mt-2 bg-slate-200 w-fit rounded-md p-1">
            <Tab className="outline-none px-5 rounded-sm ui-selected:border ui-selected:bg-white ">Cloudinary</Tab>
            <Tab className="outline-none px-5 rounded-sm ui-selected:border ui-selected:bg-white ">S3 Object</Tab>
          </Tab.List>
            ...
            <Tab.Panel>
              <div className="w-full overflow-y-scroll h-full mt-2">
                <ul className="flex flex-row flex-wrap gap-2 w-full h-full pb-5">
                  {isLoadingS3 ? (
                    <p>Loading</p>
                  ) : imageS3.data?.length < 1 ? (
                    <p>Tidak ada gambar</p>
                  ) : (
                    imageS3.data?.map((a, i) => {
                      return (
                        <li key={i} className="relative">
                          <input
                            type="radio"
        ...
        {value?.url && <ButtonSelectImage onAddImage={onAddImage} />}
      </div>
    </div>
  );
}
